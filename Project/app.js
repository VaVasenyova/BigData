...
async function callHuggingFaceAPI() {
    ...
    const cardsInfo = currentCards.map((card, index) => {
        const position = index === 0 ? 'Прошлое/Фундамент' :
            index === 1 ? 'Настоящее/Вызов' :
                'Будущее/Совет';
        return `${index + 1}. ${position}: ${card.name}\n   - Значение: ${card.meaning}\n   - Ключевые слова: ${card.keywords.join(', ')}`;
    }).join('\n\n');

    // Создаём системный промпт для таролога
    const systemPrompt = `Ты — профессиональный таролог с 20-летним опытом. Твоя задача — дать глубокую, персонализированную и вдохновляющую интерпретацию расклада Таро на РУССКОМ языке.

Пиши в теплом, поддерживающем тоне. Избегай фатализма, подчеркивай возможность выбора и влияния на ситуацию. Используй форматирование (жирный текст, списки, разделители) для удобства чтения.`;

    // Создаём пользовательский промпт
    const userPrompt = `Категория вопроса: ${categoryName}
Тип расклада: 3 карты (Прошлое → Настоящее → Будущее)

Выпавшие карты:
${cardsInfo}

Дай подробную интерпретацию расклада:
1. Общая энергия расклада (анализ ВСЕХ карт вместе)
2. Как карты взаимодействуют друг с другом
3. Практические рекомендации (минимум 5 советов)
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (response.status === 401) {
                throw new Error('❌ Неверный API ключ. Проверьте настройки (⚙️).');
            } else if (response.status === 404) {
                throw new Error('❌ Модель не найдена. Проверьте название модели.');
            } else if (response.status === 429) {
                throw new Error('🔄 Слишком много запросов. Подождите минуту.');
            } else if (response.status === 500) {
                throw new Error('⚠️ Ошибка сервера. Попробуйте позже.');
            }
            throw new Error(`Ошибка API: ${response.status} - ${errorData.error?.message || 'Unknown error'}`);
        }

        const result = await response.json();

        // Парсим ответ в формате chat completions
